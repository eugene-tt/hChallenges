...
namespace NConsoleMenu
{
	...
	public class CMenu : CMenuItem
	{
		...
		/// </para>
		/// </summary>
		public CMenu (string selector = null, bool addDefaultItems = true, bool immediateMode = false)
			: base (selector)
		{
			if (addDefaultItems) {
            ...
            this.ImmediateMenuMode = immediateMode;
        }
				...
				if (!ImmediateMenuMode) {
					...
					if (input == null)
                    {
						...
						foreach (var it in this) {
							...
							OnWriteLine ($"{i,2} {it.Selector.TrimStart('&')}");
						}
						for (;;) {
